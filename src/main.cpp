...
#if defined(NDEBUG)
# error "Sourcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x2ade20011ec3c4a832a155036c0bc629dc4cdaa3dbf6cd81f86e50d09dfe638d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sourcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sourcoin Signed Message:\n";
...
{
    // Sourcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Sourcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Sourcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Sourcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sourcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sourcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xb2472626339580288cf5bc391bf470232906e77ebb8d1b460f94685710b24bed");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Histórica reunión entre Corea del Norte y Corea del Sur";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex(" 04293584d178f80ccac225db7c42a8fad7a4f19d7c6396a0783e324321d8c77cc8d2e56d22e3bac855d805255622767dd13c0c053e3a586c1cc17cd52a115e5403") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515510984;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087477694;

        if (fTestNet)
        {
            block.nTime    = 1515510966;
            block.nNonce   = 2086782121;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x7471f795620d57f443c1cedf225ede5d2eaf6d690ce870e345b7f40d191e8522"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Sourcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SourcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SourcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SourcoinMiner(CWallet *pwallet)
{
    printf("SourcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sourcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SourcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SourcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SourcoinMiner, pwallet));
}
